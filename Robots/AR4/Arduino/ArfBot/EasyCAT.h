                                                   ...
                                                   // are meaningful, starting from LsByte                                                  
{
  ...
  while(TempLong.Byte[3] & ECAT_CSR_BUSY && (j != Tout));                  //
	if (j == Tout) {
		#ifdef DEB
			Serial.println("Tout");
		#endif 
                                      ...
                                      // that will be use by our application to write the outputs
{
  ...
  unsigned short j;
    ...
    while (TempLong.Byte[1] != (FST_BYTE_NUM_ROUND_OUT/4) && (j != Tout));       // *CCC* 
	if (j == Tout) {
		#ifdef DEB
			Serial.println("Tout");
		#endif 
    ...
    while (TempLong.Byte[1] != SEC_BYTE_NUM_ROUND_OUT/4 && (j != Tout));       // *CCC*  
	if (j == Tout) {
		#ifdef DEB
			Serial.println("Tout");
		#endif 
                                       ...
                                       // application and that will be sent to the EtherCAT master
{
  ...
  unsigned short j;
    ...
    while (TempLong.Byte[1] <   (FST_BYTE_NUM_ROUND_IN/4) && (j != Tout));       //    *CCC*
	if (j == Tout) {
		#ifdef DEB
			Serial.println("Tout");
		#endif 
    ...
    while (TempLong.Byte[1] < (SEC_BYTE_NUM_ROUND_IN/4) && (j != Tout));       //   *CCC*
	if (j == Tout) {
		#ifdef DEB
			Serial.println("Tout");
		#endif 
